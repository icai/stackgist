 ...
 * Email Service
 * based on
 * https://github.com/cnodejs/egg-cnode/blob/master/app/service/mail.js
 */
export default class MailService extends Service {
  async sendMail(data) {
    ...
    const transporter = mailer.createTransport(smtpTransport(config.mailOpts));
  ...
  async sendActiveMail(who, token, name) {
    const { config } = this;
    const from = `${config.name} <${config.mailOpts.auth.user}>`;
    const to = who;
